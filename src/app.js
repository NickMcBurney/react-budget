...
var Calculator = React.createClass({
	...
	removeInput: function () {
		console.log(this.handleInputConfimed)
		 ...
		 <fieldset>
			 ...
			 <InputField label="to this" key="1" ref="incoming" onSubmit={this.handleInputConfimed} />
			 {
				 inputs.map(function (result) {
					 console.log(result)
					 return <InputField key={result + 2} ref="incoming" label="New incoming"  onSubmit={thisElm.handleInputConfimed} />;
				 })
			 }
			 <button type="button" onClick={this.addInput}>Add input</button>
			 <button type="button" onClick={this.removeInput} className={this.state.inputCount > 0 ? '' : 'hide'}>Remove input</button>
